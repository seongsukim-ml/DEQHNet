...
def criterion_test(outputs, target, names):
    ...
    for key in names:
        if key == "orbital_coefficients":
            error_dict[key] = (
                torch.cosine_similarity(outputs[key], target[key], dim=1).abs().mean()
            )
        else:
            diff = outputs[key] - target[key]
            mae = torch.mean(torch.abs(diff))
...
class LitModel(pl.LightningModule):
    ...
    def corrupt(self, batch, mul=1):
        ...
        batch.t = batch_t
        batch.init_ham_t = (
            random_ham * (1 - batch_t.reshape(-1, 1, 1))
    ...
    def sample(self, batch, num_timesteps=150, min_t=0.01):
        device = self.model.device
        # Create a list of timesteps for the trajectory
...
@torch.no_grad()
def test_over_dataset(test_data_loader, pl_model, default_type):
    pl_model.eval()
    ...
    for idx, batch in tqdm(enumerate(test_data_loader)):
        ...
        batch = batch.to(pl_model.model.device)
        tic = time.time()
        ham = batch.hamiltonian.cpu()

        outputs, traj, _ = pl_model.sample(batch, num_timesteps=30)
        ...
        batch.orbital_energies, batch.orbital_coefficients = cal_orbital_and_energies(
            batch["overlap"], batch["hamiltonian"]
        )
        ...
        error_dict = criterion_test(outputs, batch, loss_weights)
...
def main(conf):
    ...
    lit_model = LitModel.load_from_checkpoint(conf.model_path, conf=conf)
    logger.info("Model loaded")
    # Create the PyTorch Lightning Trainer.
    warnings.filterwarnings("ignore")
    logger.info("Testing...")
    errors, h_output = test_over_dataset(test_data_loader, lit_model, default_type)
