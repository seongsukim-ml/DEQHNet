...
def validation_dataset(valid_data_loader, model, device, default_type=torch.float64):
    ...
    for valid_batch_idx, batch in enumerate(valid_data_loader):
        ...
        batch_t = sample_t(batch.hamiltonian.shape[0], device)
        batch.t = batch_t
        batch.init_ham_t = batch.init_ham * (1-batch_t.reshape(-1,1,1)) + (batch_t.reshape(-1,1,1)) * batch.hamiltonian
        outputs = model(batch, batch.init_ham_t)
