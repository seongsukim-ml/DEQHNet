...
def criterion(outputs, target, loss_weights):
    ...
    target.orbital_energies = torch.diag_embed(energy).to(target.hamiltonian.device)
    ...
    for key in loss_weights.keys():
        ...
        # loss = mse + mae
        loss = mse * (1-torch.min(target.t, torch.tensor(0.9)))
...
class LitModel(pl.LightningModule):
    def __init__(self, conf):
        ...
        self.loss_weights = {"hamiltonian": 1.0, "waloss": 2.5}
        self.default_type = (
