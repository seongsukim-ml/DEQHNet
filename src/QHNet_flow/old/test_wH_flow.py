...
def sample(
    ...
    device = batch.atoms.device
    ts = torch.linspace(min_t,1.0, num_timesteps)
    hamiltonian_traj = [batch.init_ham]
    pt = torch.zeros_like(ts[0]).repeat(batch.init_ham.shape[0]).to(device)
    batch.init_ham_t = batch.init_ham

    for i, _t in enumerate(ts):
        t = _t.repeat(batch.init_ham.shape[0]).to(device)
        batch.t = t
        outputs = model(batch, batch.init_ham_t)
        # pred_ham_t = batch.init_ham_t + outputs['hamiltonian']
        dt = t-pt
        vector_field = outputs['hamiltonian'] / (1-_t)
        ham_t = batch.init_ham_t + vector_field * dt.reshape(-1,1,1)
        hamiltonian_traj.append(ham_t)
    ...
    return res_outputs, hamiltonian_traj
        


@hydra.main(config_path='config', config_name='config')
def main(conf):
    shutil.copy(os.path.join(os.path.dirname(__file__), 'auxiliary.gbs'), hydra.core.hydra_config.HydraConfig.get().runtime.output_dir)
