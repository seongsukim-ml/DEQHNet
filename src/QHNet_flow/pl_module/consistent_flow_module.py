...
class LitModel_consistent_flow(LitModel):
    ...
    def _corrupt(self, batch, batch_t):
        ...
        if self.init_gauss:
            if self.init_gauss_center:
                random_ham = (
                    torch.randn_like(batch.hamiltonian) * self.sigma + batch.init_ham
                )
            else:
    ...
    def sample(
        ...
        batch.init_ham_t_res = torch.zeros_like(batch.init_ham)
        ...
        for idx, next_t in enumerate(lin_t[1:]):
            ...
            if self.use_res_target:
                outputs = self(batch, batch.init_ham_t_res)
            else:
                outputs = self(batch, batch.init_ham_t)
            dt = next_t - cur_t
            assert dt > 0
