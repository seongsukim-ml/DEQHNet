...
class LitModel_consistent(LitModel):
    ...
    def criterion_consistent(outputs_list, target, loss_weights):
        ...
        diff["hamiltonian"] = ham_K - (target["hamiltonian"] - target["init_ham"]) / K
    ...
    def _forward_H_and_K(self, batch):
        ...
        outputs = self(
            batch_mul,
            torch.stack([batch.hamiltonian_t, batch.init_ham]).to(device),
            torch.stack([batch.h, batch.K]).to(device),
        )
        outputs_h = {
            "hamiltonian": outputs["hamiltonian"][: batch.hamiltonian_t.shape[0]],
        }
        outputs_K = {
            "hamiltonian": outputs["hamiltonian"][batch.hamiltonian_t.shape[0] :],
        }
