...
class LitModel_flow_cf(LitModel):
    ...
    def corrupt(self, batch, mul=1, sigma=0.01):
        ...
        batch.init_ham_t = batch.init_ham_t + sigma * torch.randn_like(batch.init_ham_t)
        return batch
    ...
    def criterion(outputs, target, loss_weights, use_t_scale=False):
        ...
        for key in loss_weights.keys():
            scale = 1
            if key == "hamiltonian":
                diff = outputs[key] - (target[key] - target.random_ham)
                if use_t_scale:
    ...
    def sample(
        ...
        for idx, next_t in enumerate(lin_t[1:]):
            ...
            vector_field = outputs["hamiltonian"]
