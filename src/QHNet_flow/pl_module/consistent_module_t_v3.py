...
class LitModel_consistent_v3(LitModel):
    ...
    def batch_repeat(batch, mul=1, default_type=torch.float64, repeat_style="repeat"):
        ...
        return LitModel_consistent_v3.post_processing(
    ...
    # @staticmethod
    # def sample_t_and_h(num_batch, max_T, device, low=0):
    #     # Sample t in [0, max_T)
    #     t_batch = torch.randint(low=low, high=(max_T), size=(num_batch,)).to(device)
    #     max_h = max_T - t_batch
    ...
    #         batch.hamiltonian.shape[0], self.max_T, device, low=2
    #     )
    ...
    def consistent_sample(self, batch, mul=1, low=0.25, high=1):
        ...
        batch.t_0 = torch.zeros_like(batch.t).to(device).double()
        batch.t_1 = torch.ones_like(batch.t).to(device).double()
        batch.hamiltonian_t = batch.hamiltonian_traj[t_int]

        return batch

    @staticmethod
    def criterion_refine(outputs_dict, target, loss_weights, loss_type=1, mul=0):
        error_dict = {"loss": 0}
        diff = {}
        dict_keys = {
            "consistent": "K",
        ...
        if "K" in outputs_dict.keys():
            ham_K = outputs_dict["K"]["hamiltonian"]
            diff["consistent"] = ham_K - (
                target["hamiltonian"] - target["hamiltonian_t"]
            )

        if "O" in outputs_dict.keys():
            ham_O = outputs_dict["O"]["hamiltonian"]
            diff["hamiltonian"] = ham_O - (target["hamiltonian"] - target["init_ham"])

        if "T" in outputs_dict.keys():
            ham_T = outputs_dict["T"]["hamiltonian"]
            diff["two-step"] = ham_T - (target["hamiltonian"] - ham_O)
    ...
    def forward(self, batch, cur_H, cur_t, step=None):
    ...
    def _forward_H_and_K(self, batch):
        ...
        outputs_O = {"hamiltonian": outputs["hamiltonian"][batch.hamiltonian_t.shape[0] :]}
        # fmt: on
        return outputs_h, outputs_O
    ...
    def training_step(self, batch, batch_idx):
        ...
        outputs, outputs_O = self._forward_H_and_K(batch)
        inp = {"K": outputs, "O": outputs_O}
        if self.loss_weights.get("two-step_mae", 0) > 0:
            input_ham = outputs_O["hamiltonian"].clone().detach() + batch.init_ham
            outputs_T = self.forward(batch_one, input_ham, batch.t_1)
            inp = {"K": outputs, "O": outputs_O, "T": outputs_T}
        errors = self.criterion_refine(
            inp,
            batch,
            loss_weights=self.loss_weights,
            loss_type=self.loss_type,
            mul=self.global_step / self.trainer.max_steps,
        )
        loss = errors["loss"]
    ...
    def validation_step(self, batch, batch_idx):
        ...
        batch = self.consistent_sample(batch, self.batch_mul)
        outputs, outputs_O = self._forward_H_and_K(batch)
        input_ham = outputs_O["hamiltonian"] + batch.init_ham
        outputs_T = self.forward(batch_one, input_ham, batch.t_1)

        errors = self.criterion_refine(
            {"K": outputs, "O": outputs_O, "T": outputs_T},
            batch,
            loss_weights=self.loss_weights,
            loss_type=self.loss_type,
            mul=self.global_step / self.trainer.max_steps,
    ...
    def sample(self, batch, type="ode", num_timesteps=1):
        assert type in ["ode", "one-step", "two-step"]
        if type == "ode":
            return self._sample(batch, num_timesteps)
        elif type == "one-step":
            output = self.forward(
                batch,
                batch.init_ham,
                torch.zeros((batch.init_ham.shape[0],)).to(self.model.device),
            )
            ...
            output = self.forward(
                batch,
                output["hamiltonian"],
                torch.ones((batch.init_ham.shape[0],)).to(self.model.device),
            )
            output["hamiltonian"] = output["hamiltonian"] + batch.init_ham
            return output, None, None
    ...
    def _log_sample_error(
        self, batch_one, prefix, num_timesteps=1, post_fix="", type="one-step"
    ):
        try:
            sample, traj, pred = self.sample(
                batch_one, num_timesteps=num_timesteps, type=type
            )
