...
@hydra.main(config_path="../config_consistent", config_name="config_refinement")
def main(conf):
    ...
    pl_model_inf_cls = get_pl_model(conf.inf_model)
    logger.info(f"Using inf model: {pl_model_inf_cls}")
    pl_model_inf = pl_model_inf_cls(conf.inf_model)
    logger.info(f"Loading inf model from {conf.inf_model.ckpt_path}")
    pl_model_inf = pl_model_inf.load_from_checkpoint(
        conf.inf_model.ckpt_path, conf=conf.inf_model
    )
    
    pl_model_cls = get_pl_model(conf)
    logger.info(f"Using model: {pl_model_cls}")
    lit_model = pl_model_cls(conf, pl_model_inf)
