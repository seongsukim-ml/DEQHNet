...
class QHNet_flow(nn.Module):
    def __init__(
        ...
        **deq_kwargs,
        ...
        self.use_block_S = False
        if self.use_block_S:
            self.blocks_S = torch.nn.ModuleList()
        self.use_block_H = False
        if self.use_block_H:
        ...
        # self.linear_so2 = torch.nn.ModuleList()
        ...
        for i in range(self.num_gnn_layers):
            ...
            # self.linear_so2.append(
            #     Linear(
            #         irreps_in=self.irrep_so2_in_node,
            #         irreps_out=self.irrep_so2_in_node,
        ...
        for j in range(max(self.lmax_list) + 1):
            for i in range(self.sphere_channels):
                offset = i * (self.lmax_list[0] + 1) ** 2
                self.so2_reindex.append(
                    torch.arange(j**2 + offset, j**2 + (2 * j + 1) + offset)
                )
        self.so2_reindex = torch.cat(self.so2_reindex, dim=0)
        ...
        self.output_ii = lambda x: x
        self.output_ij = lambda x: x
    ...
    def filter(
        ...
        for i in range(self.num_resolutions):
            x.embedding[:, offset_res, :] = node_attr_R[
        ...
        for layer_idx, layer in enumerate(self.e3_gnn_layer):
            ...
            if layer_idx != 0:
                x.embedding = x_old.embedding + x.embedding
            x_old = x

            node_attr_R = x.embedding.flatten(1, 2)
            # node_attr_R = x.embedding.flatten(1, 2)[:, self.so2_reindex]
            # node_attr_R = self.linear_so2[layer_idx](node_attr_R)
