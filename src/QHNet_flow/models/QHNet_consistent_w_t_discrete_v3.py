...
class QHNet_consistent(nn.Module):
    def __init__(
        ...
        **deq_kwargs,
        ...
        self.max_T = max_T
        self.discrete_time_embedding_T = nn.Embedding(self.max_T+1, self.hs)
        self.discrete_time_embedding_H = nn.Embedding(self.max_T+1, self.hs)
        ...
        for i in range(self.num_gnn_layers):
            ...
            self.blocks_Linear.append(Linear(self.hidden_irrep_concat, self.hidden_irrep))
    ...
    def filter(
        ...
        if torch.is_floating_point(cur_t):
            ...
            int_t = torch.clamp(int_t, 0, self.max_T)
        else:
            int_t = cur_t
        ...
        # )
        node_attr_R = torch.cat(
            [node_attr_R, embedded_t[data.batch]], dim=-1
        )
        ...
        for layer_idx, layer in enumerate(self.e3_gnn_layer):
            ...
            node_concat  = torch.cat(node_concat, dim=-1).index_select(-1, self.hidden_irrep_concat_idx.to(node_attr_R.device))
