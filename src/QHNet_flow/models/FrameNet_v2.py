...
class FrameNet_V2(nn.Module):
    def __init__(
        ...
        **deq_kwargs,
        ...
        print("################FrameNet_v2##################")
        super(FrameNet_V2, self).__init__()
        self.hs = hidden_size
        self.hbs = 32
        ...
        for j in range(3):
            for i in range(self.hbs):
                self.reshape_index.append(torch.arange(self.reshape_helper[j][0]+self.ham_dim*i, self.reshape_helper[j][1]+self.ham_dim*i))
    ...
    def filter(
        ...
        for i in range(len(self.node_layers)):
            node_attr_R = self.node_layers[i](
        ...
        # import pdb
        # pdb.set_trace()
        fii = fii.reshape(fii.shape[0], -1).index_select(-1, self.reshape_index.cuda()).contiguous()
        fij = fij.reshape(fij.shape[0], -1).index_select(-1, self.reshape_index.cuda()).contiguous()
    ...
    def forward(self, data, H, keep_blocks=False, frame=True):
        ...
        H_pred = self.filter(
            H, data, node_attr, edge_vec, rbf_new, transpose_edge_index, keep_blocks, frame
        )
