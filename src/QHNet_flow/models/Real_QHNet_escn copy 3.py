...
class QHNet(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.distance_resolution = 0.02
        self.num_gaussians = int(self.max_radius / self.distance_resolution)
    ...
    def forward(self, data, H=None, keep_blocks=False):
        ...
        ### eSCN
        edge_rot_mat = self._init_edge_rot_mat(data.pos, edge_index, data.edge_vec)
        ...
        for i in range(self.num_resolutions):
            x.embedding[:, offset_res, :] = self.sphere_embedding(data.atoms.squeeze())[
        ...
        for layer_idx, layer in enumerate(self.e3_gnn_layer):
            ...
            x = layer(
                data, x, SO3_edge_rot=self.SO3_edge_rot, mappingReduced=mappingReduced
            )

            if layer_idx > self.start_layer:
                node_attr_1 = x.embedding.flatten(1, 2) * 100
                # node_attr_1 = node_attr_1[:, self.concat_idx] * 0.001
