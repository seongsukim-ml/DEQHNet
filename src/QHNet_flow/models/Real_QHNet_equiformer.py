    ...
    def forward(self, data, H=None, keep_blocks=False):
        ...
        for i in range(self.num_resolutions):
            if self.num_resolutions == 1:
                ...
                x.embedding[:, offset_res, :] = node_attr
            else:
                raise NotImplementedError
        ...
        for layer_idx, layer in enumerate(self.e3_gnn_layer):
            ...
            if layer_idx > self.start_layer:
                ...
                node_attr_1 = x.embedding.flatten(1, 2)
                # node_attr_1 = node_attr_1[:, self.concat_idx].contiguous()
                # node_attr_1 = node_attr_1.index_select(
                #     -1, self.concat_idx.to(self.device)
                # ).contiguous()
