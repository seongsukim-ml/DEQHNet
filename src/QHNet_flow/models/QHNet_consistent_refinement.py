...
class QHNet_consistent_refinement(nn.Module):
    def __init__(
        self,
        in_node_features=1,
        sh_lmax=4,
        ...
        **kwargs,
        ...
        super(QHNet_consistent_refinement, self).__init__()
        self.order = sh_lmax
        ...
        #     nn.Linear(3 * self.hs, self.hs), nn.SiLU(), nn.Linear(self.hs, self.hs)
        # )
        self.sigma_embedding = nn.Sequential(
    ...
    def filter(
        ...
        if self.use_block_S:
            ...
            node_feats_S = torch.zeros_like(node_feats_H)
        ...
        for layer_idx, layer in enumerate(self.e3_gnn_layer):
            ...
            # node_attr = torch.ones_like(node_feats_H_init.narrow(1, 0, 1))
            node_attr = node_feats_H_init.narrow(1, 0, 1)
            ...
            if self.use_block_S:
                ...
                node_feats_S = torch.zeros_like(node_feats_H)
            node_attr = self.norm(
                node_attr_R + node_feats_H + node_feats_S,
                batch=data.batch,
            )
