...
class NodeMessagePassing(MessagePassing):
    def __init__(self, hidden_size, **kwargs):
        ...
        self.hidden_size = hidden_size
        self.linear_edge = nn.Linear(3 * hidden_size, hidden_size // 2)
        self.linear_mid = nn.Linear(hidden_size //2, hidden_size//2)
...
class SelfInteractionBlockAttention(MessagePassing):
    def __init__(self, hidden_size, ham_hidden, attn=True, **kwargs):
...
class PairInteractionBlock(nn.Module):
    def __init__(self, hidden_size, ham_hidden, **kwargs):
        ...
        self.linear = nn.Linear(2 * hidden_size + ham_hidden, ham_hidden)
        self.act = nn.SiLU()
        self.linear_out2 = nn.Linear(ham_hidden, ham_hidden)
...
class HamiltonianAttention(nn.Module):
    ...
    def forward(self, ham_query, attn_bias):
        ...
        q = q.contiguous().view(-1, self.heads, self.head_dim).transpose(0,1) * self.scailing
...
class HamiltonianBlock(nn.Module):
    def __init__(self, ham_hidden, ffn_hidden, head_dim, heads,**kwargs):
        ...
        self.self_attn = HamiltonianAttention(self.ham_hidden, self.head_dim, self.heads)
...
class FrameNet(nn.Module):
    def __init__(
        ...
        **deq_kwargs,
        ...
        # self.self_bias_proj = nn.Sequential(
        #     nn.Linear(self.ham_hidden, self.ham_hidden),
        #     nn.SiLU(),
        #     nn.Linear(self.ham_hidden, self.head_dim*self.head_dim),
        # )
        ...
        for i in range(3):
            self.self_ham_attention.append(
                HamiltonianBlock(self.ham_hidden,self.ham_hidden, self.head_dim, self.num_heads)
            )
        ...
        #     nn.Linear(self.ham_hidden, self.ham_hidden),
        #     nn.SiLU(),
        #     nn.Linear(self.ham_hidden, self.num_heads*self.ham_dim),
        # )
    ...
    def filter(
        ...
        start_idx = self.num_gnn_layers-len(self.pair_ham_attention)
        for i in range(self.num_gnn_layers):
            ...
            if i > start_idx:
                fii = self.self_ham_attention[i-start_idx](fii, None)
                fij = self.pair_ham_attention[i-start_idx](fij, None)
        ...
        hamiltonian_diagonal_matrix = self.self_decoder(fii).reshape(fii.shape[0], -1,self.ham_dim, self.ham_dim)
        hamiltonian_non_diagonal_matrix = self.pair_decoder(fij).reshape(fij.shape[0], -1,self.ham_dim, self.ham_dim)
        ...
        diag_ham_scale = F.softmax(diag_ham_scale, dim=-1).reshape(fii.shape[0], -1, self.ham_dim, self.ham_dim)
        
        # non_diag_ham_scale = torch.einsum("bhi,bhj->bhij",fij_vec,fij_vec).reshape(fij.shape[0], -1, self.ham_dim*self.ham_dim)
        non_diag_ham_scale = fij_vec.reshape(fij.shape[0], -1, self.ham_dim*self.ham_dim)
        non_diag_ham_scale = F.softmax(non_diag_ham_scale, dim=-1).reshape(fij.shape[0], -1, self.ham_dim, self.ham_dim)
        
        hamiltonian_diagonal_matrix = torch.einsum("bhij,bhij->bij",hamiltonian_diagonal_matrix,diag_ham_scale)
        hamiltonian_non_diagonal_matrix = torch.einsum("bhij,bhij->bij",hamiltonian_non_diagonal_matrix,non_diag_ham_scale)

        # hamiltonian_diagonal_matrix = self.ham_decoder_self(fii)
        # hamiltonian_non_diagonal_matrix = self.ham_decoder_pair(fij)
        # hamiltonian_diagonal_matrix = hamiltonian_diagonal_matrix.reshape(
        #     fii.shape[0], -1, self.ham_dim
        # )
        # hamiltonian_non_diagonal_matrix = hamiltonian_non_diagonal_matrix.reshape(
        #     fij.shape[0], -1, self.ham_dim
        # )
        
        # scail = 1.0 / math.sqrt(self.ham_dim)
        # hamiltonian_diagonal_matrix_query = self.ham_decoder_self_query(fii).reshape(fii.shape[0], -1, self.ham_dim) * scail
        # hamiltonian_diagonal_matrix_key = self.ham_decoder_self_key(fii).reshape(fii.shape[0], -1, self.ham_dim)
        # hamiltonian_diagonal_matrix_attn_weight = torch.einsum("bhi,bhj->bij", hamiltonian_diagonal_matrix_query, hamiltonian_diagonal_matrix_key)
        # hamiltonian_diagonal_matrix_attn_weight = F.softmax(hamiltonian_diagonal_matrix_attn_weight, dim=-1)
        
        # hamiltonian_diagonal_matrix = torch.einsum("bhi,bhj->bij",hamiltonian_diagonal_matrix,hamiltonian_diagonal_matrix)
        # hamiltonian_diagonal_matrix = torch.einsum("bij,bjk->bik", hamiltonian_diagonal_matrix_attn_weight, hamiltonian_diagonal_matrix)


        # hamiltonian_non_diagonal_matrix_query = self.ham_decoder_pair_query(fij).reshape(fij.shape[0], -1, self.ham_dim) * scail
        # hamiltonian_non_diagonal_matrix_key = self.ham_decoder_pair_key(fij).reshape(fij.shape[0], -1, self.ham_dim)
        # hamiltonian_non_diagonal_matrix_attn_weight = torch.einsum("bhi,bhj->bij", hamiltonian_non_diagonal_matrix_query, hamiltonian_non_diagonal_matrix_key)
        # hamiltonian_non_diagonal_matrix_attn_weight = F.softmax(hamiltonian_non_diagonal_matrix_attn_weight, dim=-1)
        
        # hamiltonian_non_diagonal_matrix = torch.einsum("bhi,bhj->bij",hamiltonian_non_diagonal_matrix,hamiltonian_non_diagonal_matrix)
        # hamiltonian_non_diagonal_matrix = torch.einsum("bij,bjk->bik", hamiltonian_non_diagonal_matrix_attn_weight, hamiltonian_non_diagonal_matrix)
