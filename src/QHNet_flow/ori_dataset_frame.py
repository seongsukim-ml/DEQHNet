...
def _matrix_to_euler_angles(R, convention="ZYZ"):
    ...
    a, b, c = pytorch3d.transforms.matrix_to_euler_angles(R.T, convention=convention)
    return a, b, c
...
class MD17_DFT(InMemoryDataset):
    ...
    def post_process_data(data):
        ...
        Q, _ = generalized_qr_decomposition(pos, torch.eye(pos.size(0)).to(pos.dtype))

        pos_rot = pos @ Q
        ret_data.pos_rot = pos_rot

        WD_block_diag = WDs(data, Q)
        # print(data.atoms)
        WD_block_diag = torch.tensor(
            matrix_transform(
                WD_block_diag, data.atoms.flatten().numpy(), convention="pyscf_def2svp"
            ),
            dtype=torch.float64,
        ).unsqueeze(0)

        assert WD_block_diag.dim() == 3
        ret_data.WD_block_diag = WD_block_diag
        ret_data.frame = Q.unsqueeze(0)
