...
class QH9Stable(InMemoryDataset):
        ...
        for idx_i, atom_i in enumerate(atoms):  # (src)
            ...
            for idx_j, atom_j in enumerate(atoms):  # (dst)
                ...
                if len(matrix.size()) == 2:
                    ...
                    len_ = matrix.size(-1)
                    matrix_block = torch.zeros(self.full_orbitals, self.full_orbitals, len_).type(torch.float64)
                    matrix_block_mask = torch.zeros(self.full_orbitals, self.full_orbitals, len_).type(torch.float64)
    ...
    def get_mol(self, atoms, pos, Ham, ovlp):
        ...
        overlap = torch.tensor(
            matrix_transform(ovlp, atoms, convention='pyscf_def2svp'), dtype=torch.float64)
        diagonal_overlap, non_diagonal_overlap, _, _, _ = self.cut_matrix(overlap, atoms)

        Q = torch.stack([torch.block_diag(*[self.Q_dict[z][l] for z in atoms]) for l in range(60)]).double().numpy()
    ...
    def get(self, idx):
        ...
        with db_env.begin() as txn:
            ...
            _, num_nodes, atoms, pos, Ham, ovlp = \
            ...
            data = self.get_mol(atoms, pos, Ham, ovlp)
        db_env.close()
        return data
