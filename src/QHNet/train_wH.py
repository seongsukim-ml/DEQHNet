...
def main(conf):
    ...
    logger.info(model)
    shutil.copytree(
        os.path.join(root_path, "models"), os.path.join("outputs", "models")
    )
    ...
    scheduler = get_polynomial_decay_schedule_with_warmup(
        optimizer,
        num_warmup_steps=conf.warmup_step,
    ...
    for batch_idx in range(conf.num_training_steps + 1000):
        ...
        if batch_idx % conf.dataset.validation_batch_interval == 0:
            ...
            if conf.ema_start_epoch > -1 and epoch > conf.ema_start_epoch:
                ...
                logger.info(
                    f"ema Epoch {epoch} batch_idx {batch_idx} with hamiltonian "
            ...
            if errors["hamiltonian_mae"] < min_error:
                ...
                torch.save(
                    ...
                    f"best_no_ema_results.pt",
                )
                model = model.to(device)
            logger.info(
                f"no_ema Epoch {epoch} batch_idx {batch_idx} with hamiltonian "
                f"{errors['hamiltonian_mae']:.8f}."
            )
